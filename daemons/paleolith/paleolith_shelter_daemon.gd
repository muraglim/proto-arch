...
func daemon_init() -> void:
	...
	_night_lost_familiarity_reduction = Firm.get_value("paleolith_ledger", "night_lost_cache_reduction")
	...
	var cache: Dictionary = Firm.get_value("paleolith_location_ledger", "cache_knowledge")
	_familiarity_increment        = cache.get("increment", 0.25)
	_familiarity_decay            = cache.get("decay_per_day", 0.15)
	_familiarity_travel_reduction = cache.get("travel_reduction", 0.4)
