...
#✓ Nav.evict_back_module(..daemon origin point..) verified 2026.05.31 17:41 CDT
# // nav check case for daemon_evict_from_back
# nav_checker module_init
# nav_checker swaps -> with nav_check_module_target
# nav_check_module_target swaps -> nav_checker 
# nav_check_module_target now in back
# nav_checker module_resume
# nav_checker calls Nav.to_daemon
# daemon_init fires
# nav_check_daemon calls Nav.evict_back_module
#func module_init() -> void:
#	navchecker.text = "NavChecker"
#	status.text = "[status: init]"
#	await get_tree().process_frame
